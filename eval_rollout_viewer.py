...
def main():
    parser = argparse.ArgumentParser(description="Run rollout viewer on a selected scenario")
    parser.add_argument(
        "--scenario",
        choices=sorted(SCENARIO_MODULES.keys()),
        default=None,
        help="Scenario name to view (if omitted, an interactive selector is shown)",
    )
    parser.add_argument(
        "--model-path",
        default=None,
        help="Optional explicit model zip path to load",
    )
    parser.add_argument(
        "--model-rank",
        type=int,
